...
[R-Ladies](http://rladies.org/) is a global organization that promotes diversity in the R community. One way to do this is by raising the visibility of women* in tech, especially those who are R users, or developers.
...
## Warnings
**WARNING 1**: The `rtweet` package interacts with Twitter's API, which has a time limit of 7 days of doing it for free. If you would like to perform Twitter analyses after one week of tweets being created, you may have to [pay for this service](https://developer.twitter.com/en/pricing). The exception is when fetching all tweets from a whole timeline using `get_timeline`, but depending on which account one is analyzing, the amount of tweets received could be overwhelming, so use at your own risk! It works for @[rladies_iwd2018](http://www.twitter/rladies_iwd2018) because the account has relatively few tweets (n = 442 at the time of this analysis).
...
<!--- Abi: it would be nice to expand on this a bit more eg reference the authors and explain what you used from each of these blogs?--->
...
## When were most of the tweets posted?
International Women's Day 2018 took place on March 8th, 2018. I'm assuming that most tweets came out on that day, but I do know that some were tweeted one day before and one day after (likely accounting for time zone differences). Also, some of the replies were still being made a few days later.
